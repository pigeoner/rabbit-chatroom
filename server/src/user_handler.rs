...
impl UserHandler {
    ...
    pub async fn signup(&self, user: &User) -> Result<()> {
        self.db_handler.insert_user(user).await.or_else(|e| Err(anyhow!("注册失败：{}", e)))?;
        Ok(())
    }

    pub async fn login(&self, user: &User) -> Result<()> {
        let db_user = self.db_handler.get_user(&user.username).await.or_else(|e| Err(anyhow!("登录失败：{}", e)))?;
        match db_user {
            Some(db_user) => {
